...
class LimitedDataBaseArrayList : ArrayList<InfoTranslation>() {
    ...
    override fun add(element: InfoTranslation): Boolean {
        ...
        App.instance.modelCache.setCache(element).subscribeBy(onComplete = { Log.i("Logs", "Элемент добавлен!!!")})
    ...
    override fun remove(element: InfoTranslation): Boolean {
        if (!super.contains(element))
            return false
        App.instance.modelCache.deleteCache(element).subscribeBy(onComplete = { Log.i("Logs", "Элемент удалён!!!")})
